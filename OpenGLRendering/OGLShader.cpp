...
OGLShader::OGLShader(const string &vertex, const string &fragment,
                     const string &geometry, const string &domain,
                     const string &hull)
...
void OGLShader::ReloadShader() {
  ...
  for (int i = 0; i < (int)ShaderStages::MAX_SIZE; ++i) {
    if (!shaderFiles[i].empty()) {
      if (Assets::ReadTextFile(Assets::SHADERDIR + shaderFiles[i],
                               fileContents)) {
        Preprocessor(fileContents);

        shaderIDs[i] = glCreateShader(shaderTypes[i]);

        GL_DEBUG("Reading {} shader {}.", shaderNames[i], shaderFiles[i]);
        ...
        if (shaderValid[i] == GL_TRUE) {
          ...
          GL_ERROR("{} shader {} failed to load.", shaderNames[i],
                   shaderFiles[i]);
        }
  ...
  if (programValid != GL_TRUE) {
    GL_ERROR("This shader has failed!");
  } else {
    GL_TRACE("Shader loaded!");
  }
}
...
bool OGLShader::Preprocessor(string &shaderFile) {
  ...
  while (true) {
    ...
    std::string_view substr =
        std::string_view(shaderFile).substr(lineStart, lineLength);
    ...
    if (hasComment == string::npos && hasInclude != string::npos) {
      ...
      std::string filename =
          std::string(substr.substr(nameStart + 1, nameEnd - nameStart - 1));

      auto search = includes.find(filename);

      if (search != includes.end()) {
        GL_ERROR("Preprocessor failure! Recursive inclusion of file {}",
                 filename);
        return false;
      }
      ...
      if (!Assets::ReadTextFile(Assets::SHADERDIR + filename, newFile)) {
        GL_ERROR("Preprocessor failure! No include file: {}", filename);
        return false;
      } else {
        shaderFile.replace(lineStart, lineLength, newFile);
        continue; // Don't move on, the replaced file could have additional
                  // includes!
      }
    }
    lineStart = lineEnd + 1;
