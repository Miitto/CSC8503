...
OGLRenderer::OGLRenderer(Window &w) : RendererBase(w) {
  ...
  ImGui::CreateContext();
#ifdef _WIN32
  NCL::Win32Code::Win32Window *realWindow = (NCL::Win32Code::Win32Window *)&w;
...
void OGLRenderer::SwapBuffers() { ::SwapBuffers(deviceContext); }
...
void OGLRenderer::DrawBoundMesh(int subLayer, int numInstances) {
  ...
  static GLenum primitiveLookup[] = {GL_POINTS,         GL_LINES,
  ...
  if (numInstances > 1) {
    if (boundMesh->GetIndexCount() > 0) {
      glDrawElementsInstanced(mode, count, GL_UNSIGNED_INT,
                              (const GLvoid *)(offset * sizeof(unsigned int)),
                              numInstances);
    } else {
...
void OGLRenderer::BindTextureToShader(const OGLTexture &t,
  ...
  GLuint slot =
      glGetUniformLocation(activeShader->GetProgramID(), uniform.c_str());
...
void OGLRenderer::InitWithWin32(Window &w) {
  ...
  // A pixel format descriptor is a struct that tells the Windows OS what type
  // of front / back buffers we want to create etc
  ...
  pfd.dwFlags =
      ...
      PFD_DRAW_TO_WINDOW; // It must be double buffered, it must support OGL(!),
                          // and it must allow us to draw to it...
  pfd.iPixelType =
      PFD_TYPE_RGBA;    // We want our front / back buffer to have 4 channels!
  ...
  if (!(PixelFormat = ChoosePixelFormat(
            deviceContext,
            &pfd))) { // Did Windows Find A Matching Pixel Format for our PFD?
  ...
  HGLRC tempContext; // We need a temporary OpenGL context to check for
                     // OpenGL 3.2 compatibility...stupid!!!
  if (!(tempContext = wglCreateContext(
            deviceContext))) { // Are We Able To get the temporary context?
  ...
  if (!wglMakeCurrent(deviceContext,
                      tempContext)) { // Try To Activate The Rendering Context
  ...
  if (!gladLoaderLoadGL()) {
    std::cout << __FUNCTION__
              << " Cannot initialise GLAD!\n"; // It's all gone wrong!
    return;
  }
  // Now we have a temporary context, we can find out if we support OGL 4.x
  char *ver =
      (char *)glGetString(GL_VERSION); // ver must equal "4.1.0" (or greater!)
  int major =
      ver[0] -
      '0'; // casts the 'correct' major version integer from our version string
  int minor =
      ver[2] -
      '0'; // casts the 'correct' minor version integer from our version string
  ...
  if (major == 4 &&
      minor < 1) { // Graphics hardware does not support ENOUGH of OGL 4! Erk...
    std::cout << __FUNCTION__
              << " Device does not support at least OpenGL 4.1!\n";
    wglDeleteContext(tempContext);
    return;
  }
  ...
  // Everywhere else in the Renderers, we use function pointers provided by
  // GLEW...but we can't initialise GLEW yet! So we have to use the 'Wiggle' API
...
void OGLRenderer::DestroyWithWin32() { wglDeleteContext(renderContext); }
...
OGLDebugScope::OGLDebugScope(const std::string &scope) {
  glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION, 0, scope.length(),
                   scope.c_str());
}
OGLDebugScope::~OGLDebugScope() { glPopDebugGroup(); }