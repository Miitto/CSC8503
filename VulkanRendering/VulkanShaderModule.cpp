...
VulkanShaderModule::VulkanShaderModule(const std::string &filename,
  ...
  if (dataSize > 0) {
    m_shaderModule = device.createShaderModuleUnique(
        {.flags = {}, .codeSize = dataSize, .pCode = (uint32_t *)data});

    AddReflectionData(dataSize, data, stage);
    BuildLayouts(device);

    Vulkan::SetDebugName(device, vk::ObjectType::eShaderModule,
                         Vulkan::GetVulkanHandle(*m_shaderModule), filename);
  } else {
...
void VulkanShaderModule::CombineLayoutBindings(
    std::vector<std::vector<vk::DescriptorSetLayoutBinding>> &inoutBindings)
    const {
  const int numSets =
      std::max(inoutBindings.size(), m_allLayoutsBindings.size());
  ...
  for (int i = 0; i < m_allLayoutsBindings.size(); ++i) {
    ...
    const std::vector<vk::DescriptorSetLayoutBinding> &baseSet =
        m_allLayoutsBindings[i];
    ...
    for (int j = 0; j < baseSet.size(); ++j) {
      if (baseSet[j].stageFlags != vk::ShaderStageFlags()) {
        ...
        outSet[j].stageFlags |=
            m_shaderStage; // Combine sets across shader stages
      }
    }
  }
}

void VulkanShaderModule::CombinePushConstantRanges(
    std::vector<vk::PushConstantRange> &inoutRanges) const {
...
void VulkanShaderModule::AddReflectionData(uint32_t dataSize, const void *data,
                                           vk::ShaderStageFlags stage) {
  SpvReflectShaderModule module;
  SpvReflectResult result =
      spvReflectCreateShaderModule(dataSize, data, &module);
  ...
  result = spvReflectEnumerateDescriptorSets(&module, &descriptorCount,
  ...
  for (auto &set : descriptorSetLayouts) {
    ...
    std::vector<vk::DescriptorSetLayoutBinding> &setLayout =
        m_allLayoutsBindings[set->set];
    ...
    for (int i = 0; i < set->binding_count; ++i) {
      ...
      if (setLayout[index].stageFlags != vk::ShaderStageFlags()) {
        // Check that something hasn't gone wrong with the binding combo!
        if (setLayout[index].descriptorType !=
      ...
      setLayout[index].descriptorType =
          (vk::DescriptorType)binding->descriptor_type;
  ...
  result =
      spvReflectEnumeratePushConstantBlocks(&module, &pushConstantCount, NULL);
  assert(result == SPV_REFLECT_RESULT_SUCCESS);
