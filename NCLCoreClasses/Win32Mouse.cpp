...
void Win32Mouse::UpdateRAW(RAWINPUT *raw) {
  if (isAwake) {
    ...
    if (isAbsolute) {
      ...
      absolutePosition.x =
          (raw->data.mouse.lLastX / (float)USHRT_MAX) * screenWidth;
      absolutePosition.y =
          (raw->data.mouse.lLastY / (float)USHRT_MAX) * screenHeight;

      if (setAbsolute) {
        relativePosition.x =
            (absolutePosition.x - prevAbsolute.x) * sensitivity;
        relativePosition.y =
            (absolutePosition.y - prevAbsolute.y) * sensitivity;
      }
      setAbsolute = true;
    } else {
      /*
      ...
      */
      ...
      /*
      Bounds check the absolute position of the mouse, so it doesn't disappear
      off screen edges...
      */
      absolutePosition.x =
          std::clamp(absolutePosition.x, 0.0f, (float)absolutePositionBounds.x);
      absolutePosition.y =
          std::clamp(absolutePosition.y, 0.0f, (float)absolutePositionBounds.y);
    }

    auto &io = ImGui::GetIO();

    // Ignore mouse wheel events if ImGui wants to capture the mouse.
    ...
    */
    ...
    static int buttonps[5] = {RI_MOUSE_BUTTON_1_UP, RI_MOUSE_BUTTON_2_UP,
                              RI_MOUSE_BUTTON_3_UP, RI_MOUSE_BUTTON_4_UP,
                              RI_MOUSE_BUTTON_5_UP};

    for (int i = 0; i < 5; ++i) {
      if (raw->data.mouse.usButtonFlags & buttondowns[i]) {
        // Ignore mouse down events if ImGui wants to capture the mouse.
        if (io.WantCaptureMouse)
          continue;
        ...
        /*
        If it wasn't too long ago since we last clicked, we trigger a double
        click!
        */
        ...
        /*
        No matter whether the mouse was double clicked or not, we reset the
        clicklimit
        */
