...
Win32Window::Win32Window(const WindowInitialisation &winInitInfo) {
  ...
  if (fullScreen) {
    ...
    dmScreenSettings.dmPelsHeight =
        winInitInfo.height;             // Selected Screen Height
  ...
  SetConsolePosition(winInitInfo.consolePositionX,
                     winInitInfo.consolePositionY);
...
Win32Window::~Win32Window(void) { init = false; }
...
LRESULT CALLBACK Win32Window::WindowProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case (WM_SIZE): {
    ...
    if (newX > 0 && newY > 0 &&
        (newX != thisWindow->size.x || newY != thisWindow->size.y)) {
  ...
  if (applyResize) {
    if (thisWindow->eventHandler) {
      thisWindow->eventHandler(NCL::WindowEvent::Resize, thisWindow->size.x,
                               thisWindow->size.y);
    }
...
void Win32Window::ShowOSPointer(bool show) {
  if (show == showMouse) {
    return; // ShowCursor does weird things, due to being a counter
            // internally...
  }
