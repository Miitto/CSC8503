...
Matrix4 biasMatrix = Matrix::Translation(Vector3(0.5f, 0.5f, 0.5f)) *
...
GameTechRenderer::GameTechRenderer(GameWorld &world)
  ...
  skyboxMesh->SetVertexPositions({Vector3(-1, 1, -1), Vector3(-1, -1, -1),
  ...
  debugTexMesh->SetVertexPositions({Vector3(-1, 1, 0), Vector3(-1, -1, 0),
...
void GameTechRenderer::LoadSkybox() {
  ...
  for (int i = 0; i < 6; ++i) {
    TextureLoader::LoadTexture(filenames[i], texData[i], width[i], height[i],
                               channels[i], flags[i]);
    if (i > 0 && (width[i] != width[0] || height[0] != height[0])) {
      std::cout << __FUNCTION__
                << " cubemap input textures don't match in size?\n";
      return;
    }
  }
  ...
  for (int i = 0; i < 6; ++i) {
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB, width[i],
                 height[i], 0, type, GL_UNSIGNED_BYTE, texData[i]);
  }
  ...
  gameWorld.OperateOnContents([&](GameObject *o) {
    if (o->IsActive()) {
      ...
      if (g) {
        ...
        ObjectSortState o;
        o.object = g;
        o.distanceFromCamera =
            Vector::LengthSquared(camPos - g->GetTransform().GetPosition());
...
void GameTechRenderer::RenderShadowMapPass(std::vector<ObjectSortState> &list) {
  ...
  int mvpLocation =
      glGetUniformLocation(shadowShader->GetProgramID(), "mvpMatrix");

  Matrix4 shadowViewMatrix = Matrix::View(gameWorld.GetSunPosition(),
...
void GameTechRenderer::RenderSkyboxPass() {
  ...
  int projLocation =
      glGetUniformLocation(skyboxShader->GetProgramID(), "projMatrix");
  int viewLocation =
      glGetUniformLocation(skyboxShader->GetProgramID(), "viewMatrix");
  int texLocation =
      glGetUniformLocation(skyboxShader->GetProgramID(), "cubeTex");
...
void GameTechRenderer::RenderOpaquePass(std::vector<ObjectSortState> &list) {
  ...
  int projLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "projMatrix");
  int viewLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "viewMatrix");
  int modelLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "modelMatrix");
  int colourLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "objectColour");
  int hasVColLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "hasVertexColours");
  int hasTexLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "hasTexture");

  int lightPosLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "sunPos");
  int lightColourLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "sunColour");
  int lightRadiusLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "sunRadius");

  int cameraLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "cameraPos");
  int shadowTexLocation =
      glGetUniformLocation(activeShader->GetProgramID(), "shadowTex");
  int shadowLocation =
...
void GameTechRenderer::RenderTransparentPass(
    std::vector<ObjectSortState> &list) {
  glEnable(GL_BLEND);
  glEnable(GL_CULL_FACE);
...
void GameTechRenderer::RenderLines() {
  ...
  GLuint texSlot =
      glGetUniformLocation(debugShader->GetProgramID(), "useTexture");
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0,
                  lines.size() * sizeof(Debug::DebugLineEntry), lines.data());
...
void GameTechRenderer::RenderText() {
  const std::vector<Debug::DebugStringEntry> &strings =
      Debug::GetDebugStrings();
  if (strings.empty()) {
    return;
  }
  ...
  for (const auto &s : strings) {
    ...
    Debug::GetDebugFont()->BuildVerticesForString(
        s.data, s.position, s.colour, size, debugTextPos, debugTextUVs,
        debugTextColours);
  }

  glBindBuffer(GL_ARRAY_BUFFER, textVertVBO);
  glBufferSubData(GL_ARRAY_BUFFER, 0, frameVertCount * sizeof(Vector3),
                  debugTextPos.data());
  glBindBuffer(GL_ARRAY_BUFFER, textColourVBO);
  glBufferSubData(GL_ARRAY_BUFFER, 0, frameVertCount * sizeof(Vector4),
                  debugTextColours.data());
  glBindBuffer(GL_ARRAY_BUFFER, textTexVBO);
  glBufferSubData(GL_ARRAY_BUFFER, 0, frameVertCount * sizeof(Vector2),
                  debugTextUVs.data());
...
void GameTechRenderer::RenderTextures() {
  ...
  GLuint useColourSlot =
      glGetUniformLocation(debugShader->GetProgramID(), "useColour");
  ...
  for (const auto &tex : texEntries) {
    ...
    Matrix4 transform =
        Matrix::Translation(Vector3(tex.position.x, tex.position.y, 0)) *
    ...
    glUniform4f(colourSlot, tex.colour.x, tex.colour.y, tex.colour.z,
                tex.colour.w);
...
void GameTechRenderer::SetDebugStringBufferSizes(size_t newVertCount) {
  if (newVertCount > textCount) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, textVertVBO);
    glBufferData(GL_ARRAY_BUFFER, textCount * sizeof(Vector3), nullptr,
                 GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, textColourVBO);
    glBufferData(GL_ARRAY_BUFFER, textCount * sizeof(Vector4), nullptr,
                 GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, textTexVBO);
    glBufferData(GL_ARRAY_BUFFER, textCount * sizeof(Vector2), nullptr,
                 GL_DYNAMIC_DRAW);
...
void GameTechRenderer::SetDebugLineBufferSizes(size_t newVertCount) {
  if (newVertCount > lineCount) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, lineVertVBO);
    glBufferData(GL_ARRAY_BUFFER, lineCount * sizeof(Debug::DebugLineEntry),
    ...
    glVertexAttribFormat(0, 3, GL_FLOAT, false,
                         offsetof(Debug::DebugLineEntry, start));
    glVertexAttribBinding(0, 0);
    glBindVertexBuffer(0, lineVertVBO, 0, realStride);

    glVertexAttribFormat(1, 4, GL_FLOAT, false,
                         offsetof(Debug::DebugLineEntry, colourA));
